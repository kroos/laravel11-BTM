...
function createPersonnels(){
	...
	var personnels_counter = {{ ($btmemailapplication->hasmanyemailgroupmember()->count())?($btmemailapplication->hasmanyemailgroupmember()->count() - 1):0 }};
...
@if($btmemailapplication->hasmanyemailgroupmember()->count())
	createPersonnels();
	<?php $ke = 0; ?>
	@foreach($btmemailapplication->hasmanyemailgroupmember()->get() as $emailmem2)
...
$(`#gemail`).change(function(){
	if(this.checked) {
		$(`#wrap_group_email`).append(
				...
				@if($btmemailapplication->hasmanyemailgroupmember()->count())
					<?php $k = 0; ?>
					@foreach($btmemailapplication->hasmanyemailgroupmember()->get() as $emailmem1)
						`<div class="col-sm-12 row mt-3">
							<div class="col-sm-11 m-0 mt-2 row">
