...
namespace App\Notifications\EmailApplication\Delete;
...
class ApplicantEmailApproverDelete extends Notification
{
	use Queueable;
	...
	public function toMail(object $notifiable): MailMessage
	{
		return (new MailMessage)
			->subject('BTM : (BTM01) Deleted Application of Email Account For Approval')
			->greeting('BTMgo')
			->line('Dear ' . $notifiable->name . ',')
			->line('This is to inform you that '.Staff::find($this->data->nostaf)->nama.', has deleted a request for email registration. The details of the application are not available in the system anymore.')
