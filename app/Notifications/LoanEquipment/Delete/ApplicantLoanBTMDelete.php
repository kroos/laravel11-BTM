...
namespace App\Notifications\LoanEquipment\Delete;
...
class ApplicantLoanBTMDelete extends Notification
{
	use Queueable;
	...
	{
		// since its deleted, then no db notification
	...
	public function toMail(object $notifiable): MailMessage
	{
		return (new MailMessage)
			->subject('BTM : (BTM03) Deleted Application of Loan Equipment For Approval')
			->greeting('BTMgo')
			->line('Dear ' . $notifiable->name . ',')
			->line('This is to inform you that '.Staff::find($this->data->nostaf)->nama.', has deleted a request for a loan application. The details of the application are not available in the system anymore.')
			...
			// ->action('View Form', route('btmloanapplications.show', $this->data->id))
