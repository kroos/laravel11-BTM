...
namespace App\Notifications\RegisterAccountICMS\Create;
...
class ApplicantICMSBTMCreate extends Notification
{
	use Queueable;
	...
	public function toMail(object $notifiable): MailMessage
	{
		return (new MailMessage)
			->subject('BTM : (BTM02) New Application of Account & ICMS Modules For Approval')
			->greeting('BTMgo')
			->line('Dear ' . $notifiable->nama . ',')
			->line('This is to inform you that '.{{ Staff::find($this->data->nostaf)->nama }}.', has submitted a request for a new account & ICMS modules registration. The details of the application are available in the system for your review.')
	...
	{
		return [
			'data' => '(BTM02) New account & ICMS modules application',
			'link' => route('btmicmsrequester.edit', $this->data->id),
		];
	}
