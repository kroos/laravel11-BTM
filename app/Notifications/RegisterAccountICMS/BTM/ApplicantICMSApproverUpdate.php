...
class ApplicantICMSApproverUpdate extends Notification
{
	use Queueable;
	...
	public function toMail(object $notifiable): MailMessage
	{
		return (new MailMessage)
			->subject('BTM : (BTM02) BTM Approval Application of Account & ICMS Modules For Approval')
			->greeting('BTMgo')
			->line('Dear ' . $notifiable->nama . ',')
			->line('This is to inform you that '.Staff::find($this->data->nostaf)->nama.' application, has been given an approval by '.Staff::find($this->data->btm_approver)->nama.' on a request for an account & ICMS modules.')
