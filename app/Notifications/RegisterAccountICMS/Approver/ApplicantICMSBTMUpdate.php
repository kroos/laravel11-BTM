...
namespace App\Notifications\RegisterAccountICMS\Approver;
...
class ApplicantICMSBTMUpdate extends Notification
{
	...
	public function toMail(object $notifiable): MailMessage
	{
		return (new MailMessage)
			->subject('BTM : (BTM03) Approval Application of Account & ICMS Modules For Approval')
			->greeting('BTMgo')
			->line('Dear ' . $notifiable->nama . ',')
			->line('This is to inform you that '.{{ Staff::find($this->data->nostaf)->nama }}.' application, has been given an approval on a request for account & ICMS modules by '.Staff::find($this->data->approver_staff)->nama.'. The details of the application are available in the system for your review.')
